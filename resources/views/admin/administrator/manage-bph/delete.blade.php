<!-- Modal Delete -->
@foreach($bphs as $bph)
    <div id="DeleteModal-{{ $bph->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
            <div class="flex flex-col items-center space-y-4">
                ...
                <div class="bg-gray-50 w-full border-2 border-red-500 rounded-lg p-4 text-sm text-left italic text-gray-500 space-y-1">
                    <p><span class="font-lg">Nama Jabatan:</span> <span class="font-medium">{{ $bph->nama }}</span> </p>
                    <p><span class="font-lg">Jenis Jabatan:</span> <span class="font-medium">{{ $bph->jenis }}</span></p>
                    <p><span class="font-lg">Parent ID:</span> <span class="font-medium">{{ $bph->parent?->nama ?? '-' }}</span></p>
                    <p><span class="font-lg">Level Jabatan:</span> <span class="font-medium">{{ $bph->level }}</span></p>
                    <p><span class="font-lg">Urutan Jabatan:</span> <span class="font-medium">{{ $bph->urutan }}</span></p>
                </div>

                <!-- Form Hapus Jabatan -->
                <form id="deleteForm-{{ $bph->id }}"
                    method="POST"
                    action="{{ route('manage-bph.destroy', $bph->id) }}">
                    @csrf
                    @method('DELETE')
                    <!-- Tombol -->
                    <div class="flex justify-center space-x-3 mt-6">
                        <!-- Tombol Batal -->
                        <button type="button"
                            onclick="closeDeleteModal('{{ $bph->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $bph->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
