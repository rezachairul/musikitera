...
<x-admin.administrator.layouts
    ...
    <section class="pb-10 overflow-x-auto">
        <div class="min-w-[1200px] tree-container">
            <ul>
                @foreach ($bphTree as $node)
                    @include('admin.administrator.manage-bph.partials.tree-node', ['node' => $node])
                @endforeach
            </ul>
        </div>
    </section>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3 sm:p-6 m-3 sm:m-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-3 sm:gap-2">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <!-- Export -->
                 <a href="{{ route('manage-bph.export', [
            ...
            <div class="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="relative w-full sm:w-1/3">
                    ...
                    <input
                        ...
                        id="search-input"
                        value="{{ $search }}"
                        data-url="{{ route('manage-bph.index') }}"
                        data-target="bphTableBody"
                ...
                <div class="flex gap-3 w-full sm:w-2/3">
                    ...
                    <select
                        ...
                        @foreach (\App\Models\admin\administrator\AdminManageBPH::ALLOWED_JENIS as $jenis)
            ...
            <div class="min-w-full inline-block align-middle">
                <div class="overflow-hidden border border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="bphTableBody" class="bg-white divide-y divide-gray-200">
                            @include ('admin.administrator.manage-bph.partials.table_body')
                        </tbody>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4 pt-4">
            ...
            <div class="text-sm text-gray-500 text-center sm:text-left">
                Menampilkan
                <span class="font-medium">{{ $bphs->firstItem() ?? 0 }}</span>
                sampai                
                <span class="font-medium">{{ $bphs->lastItem() ?? 0 }}</span>
                dari
                <span class="font-medium">{{ $bphs->total() ?? 00 }}</span> {{$title}}.
            </div>
            ...
            <div class="flex justify-center sm:justify-end">
                <nav class="inline-flex space-x-1 sm:space-x-2" aria-label="Pagination">
                    ...
                    @foreach ($bphs->getUrlRange(1, $bphs->lastPage()) as $page => $url)
                        @if ($page == $bphs->currentPage())
                    ...
                    {{-- Tombol Selanjutnya --}}
                    @if ($bphs->hasMorePages())
                        <a href="{{ $bphs->nextPageUrl() }}" 
                        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                            Selanjutnya
                        </a>
                    @else
    ...
    <!-- Modals -->
    @include('admin.administrator.manage-bph.create')
    @include('admin.administrator.manage-bph.update')
    @include('admin.administrator.manage-bph.delete')
