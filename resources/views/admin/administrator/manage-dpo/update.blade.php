<!-- Modal Edit Manage dpo Data -->
@foreach($dpos as $dpo)
<div id="UpdateModal-{{ $dpo->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 relative">
        ...
        <form id="editJabatanForm"
            method="POST"
            action="{{ route('manage-dpo.update', $dpo->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="jenis" id="jenis"
                    ...
                    @foreach ($jenisList as $value => $label)
                        <option value="{{ $value }}"
                            {{ old('jenis', $dpo->jenis) === $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
                    ...
                    @foreach ($parentJabatans as $parent)
                        @if ($parent->id !== $dpo->id)
                            <option value="{{ $parent->id }}"
                                {{ old('parent_id', $dpo->parent_id) == $parent->id ? 'selected' : '' }}>
            ...
            <div class="flex justify-end space-x-2 mt-6">
                <button type="button" onclick="closeUpdateModal('{{ $dpo->id }}')"
        ...
        <!-- Tombol X di pojok -->
        <button onclick="closeUpdateModal('{{ $dpo->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
