@forelse ($badan_penguruses as $key => $badan_pengurus)
<tr class="hover:bg-gray-50 text-left text-xs">
    <!-- No -->
    <td class="px-6 py-4 whitespace-nowrap">
        {{ $badan_penguruses->firstItem() + $key }}
    </td>
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        {{ $badan_pengurus->kabinet->nama_kabinet }} | {{ $badan_pengurus->kabinet->periode_awal }} – {{ $badan_pengurus->kabinet->periode_akhir }}
    </td>
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center align-middle">
        <div class="flex justify-center items-center h-full space-x-2">
            <!-- Edit Button -->
            <button
                onclick="openUpdateModal('{{ $badan_pengurus->id }}')"
            ...
            <!-- Delete Button -->
            <button
                onclick="openDeleteModal('{{ $badan_pengurus->id }}')"
...
<tr class="hover:bg-gray-50">
    <td colspan="8" class="px-6 py-4 text-center text-gray-500 italic">
        <div class="flex flex-col items-center justify-center text-sm text-gray-500 space-y-1">
            @if ($badan_penguruses->isEmpty() && !request()->filled('search') && !request()->filled('filter'))
                <!-- Icon Data Kosong -->
            ...
            @elseif ($badan_penguruses->isEmpty() && request()->filled('search'))
                <!-- Icon Pencarian -->
            ...
            @elseif ($badan_penguruses->isEmpty() && request()->filled('filter'))
                <!-- Icon Filter -->
