    ...
    <div id="UpdateModal-{{ $manage_pembina->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg w-full max-w-2xl p-6 relative">
           ...
           <!-- Form Update Anggota Aktif -->
            <form id="editForm-{{ $manage_pembina->id }}" method="POST" action="{{ route('anggota-aktif.update', $manage_pembina->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nim" id="edit_nim"
                            value="{{ old('nim', $manage_pembina->nim) }}"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="angkatan" id="edit_angkatan"
                            value="{{ old('angkatan', $manage_pembina->angkatan) }}"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="prodi" id="edit_prodi"
                            value="{{ old('prodi', $manage_pembina->prodi) }}"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="nomor_urut" id="edit_nomor_urut"
                            value="{{ old('nomor_urut', $manage_pembina->nomor_urut) }}"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="angkatan_ukm" id="edit_angkatan_ukm"
                            value="{{ old('angkatan_ukm', $manage_pembina->angkatan_ukm) }}"
                            class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500">
                    </div>
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="pendiri" value="1" id="edit_pendiri_yes"
                                    class="hidden peer"
                                    {{ old('pendiri', $manage_pembina->pendiri) == 1 ? 'checked' : '' }}>
                            ...
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="pendiri" value="0" id="edit_pendiri_no"
                                    class="hidden peer"
                                    {{ old('pendiri', $manage_pembina->pendiri) == 0 ? 'checked' : '' }}>
                                <span class="w-5 h-5 rounded-full border-2 border-gray-300 flex items-center justify-center 
                    ...
                    <div>
                        ...
                        <select name="status" id="edit_status"
                            ...
                            <option value="on_going" {{ old('status', $manage_pembina->status) == 'on_going' ? 'selected' : '' }}>On Going</option>
                            <option value="graduate" {{ old('status', $manage_pembina->status) == 'graduate' ? 'selected' : '' }}>Graduate</option>
                            <option value="drop_out" {{ old('status', $manage_pembina->status) == 'drop_out' ? 'selected' : '' }}>Drop Out</option>
                            <option value="exit" {{ old('status', $manage_pembina->status) == 'exit' ? 'selected' : '' }}>Exit</option>
                        </select>
                    </div>
                </div>
