<!-- Modal Delete -->
@foreach($ctas as $cta)
    <div id="DeleteModal-{{ $cta->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
            <div class="flex flex-col items-center space-y-4">
                ...
                <!-- Detail cta -->
                <div class="bg-gray-50 w-full border-2 border-red-500 rounded-lg p-4 text-sm text-gray-700 space-y-3"><!-- Foto Pendaftar -->
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="text-left">
                            <p><span class="font-semibold">Nama:</span> {{ $cta->nama_lengkap }}</p>
                            <p><span class="font-semibold">NIM:</span> {{ $cta->nim }}</p>
                            <p><span class="font-semibold">Program Studi:</span> {{ $cta->program_studi }}</p>
                            <p><span class="font-semibold">Angkatan:</span> {{ $cta->angkatan }}</p>
                        </div>
                    </div>
                </div>

                <!-- Form Hapus -->
                <form id="deleteForm-{{ $cta->id }}" method="POST" action="{{ route('manage-cta.destroy', $cta->id ) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-3 mt-4">
                        <!-- Tombol Batal -->
                        <button type="button" onclick="closeDeleteModal('{{ $cta->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $cta->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
