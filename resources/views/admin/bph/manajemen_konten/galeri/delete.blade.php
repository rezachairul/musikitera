...
@foreach($galeris as $galeri)
    <div id="DeleteModal-{{ $galeri->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
            <div class="flex flex-col items-center space-y-4">
                ...
                <!-- Detail Galeri (Delete Confirmation) -->
                <div class="bg-red-50 w-full border-2 border-red-500 rounded-lg p-4 text-sm text-red-700 space-y-3">
                    <div class="flex items-center text-left space-x-4">
                        ...
                        <div class="flex flex-col">
                            <p><span class="font-semibold">Judul:</span> {{ $galeri->title ?? '-' }}</p>
                            <p><span class="font-semibold">Deskripsi:</span> {{ $galeri->description ?? '-' }}</p>
                            <p><span class="font-semibold">Tanggal Kegiatan:</span> 
                                {{ $galeri->kegiatan_date ? \Carbon\Carbon::parse($galeri->kegiatan_date)->format('d M Y') : '-' }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Form Hapus -->
                <form id="deleteForm-{{ $galeri->id }}" method="POST" action="{{ route('manage-galeri.destroy', $galeri->id ) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-3 mt-4">
                        <!-- Tombol Batal -->
                        <button type="button" onclick="closeDeleteModal('{{ $galeri->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $galeri->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
