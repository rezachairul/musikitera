...
@foreach($heroes as $hero)
    <div id="DeleteModal-{{ $hero->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
            <div class="flex flex-col items-center space-y-4">
                ...
                <div class="bg-gray-50 w-full border-2 border-red-500 rounded-lg p-4 text-sm text-gray-700 space-y-3">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="text-left">
                            <p><span class="font-semibold">Quotes 1:</span> {{ $hero->quote_1 }}</p>
                            <p><span class="font-semibold">Quotes 2:</span> {{ $hero->quote_2 }}</p>
                        </div>
                    </div>
                </div>

                <!-- Form Hapus -->
                <form id="deleteForm-{{ $hero->id }}" method="POST" action="{{ route('manage-hero.destroy', $hero->id ) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-3 mt-4">
                        <!-- Tombol Batal -->
                        <button type="button" onclick="closeDeleteModal('{{ $hero->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $hero->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
