@forelse ($shortlinks as $key => $shortlink)
<tr class="hover:bg-gray-50 text-left text-xs">
   <!-- No -->
    <td class="px-6 py-4 whitespace-nowrap">
        {{ $shortlinks->firstItem() + $key }}
    </td>

    <!-- Short Link -->
    <td class="px-6 py-4 whitespace-nowrap">
        <a href="{{ route('shortlink.redirect', $shortlink->slug) }}" target="_blank" class="text-blue-600 underline">
    ...
    <td class="px-6 py-4 whitespace-nowrap text-center">
        <span class="{{ $shortlink->status_badge_color }}">
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center align-middle">
        <div class="flex justify-center items-center h-full space-x-2">
            <!-- Edit Button -->
            <button
                onclick="openUpdateModal('{{ $shortlink->id }}')"
            ...
            @if($shortlink->status === 'expired')
                <button
                    type="button"
                    disabled
                    class="text-gray-300 cursor-not-allowed transition"
            ...
            @else
                <button
                    type="button"
                    data-url="{{ url('/r/' . $shortlink->slug) }}"
                    class="copy-shortlink text-gray-600 hover:text-blue-600 transition"
            ...
            <!-- Delete Button -->
            <button
                onclick="openDeleteModal('{{ $shortlink->id }}')"
...
<tr class="hover:bg-gray-50">
    <td colspan="7" class="px-6 py-4 text-center text-gray-500 italic">
        <div class="flex flex-col items-center justify-center text-sm text-gray-500 space-y-1">
            @if ($shortlinks->isEmpty() && !request()->filled('search') && !request()->filled('filter'))
                <!-- Icon Data Kosong -->
            ...
            @elseif ($shortlinks->isEmpty() && request()->filled('search'))
                <!-- Icon Pencarian -->
            ...
            @elseif ($shortlinks->isEmpty() && request()->filled('filter'))
                <!-- Icon Filter -->
