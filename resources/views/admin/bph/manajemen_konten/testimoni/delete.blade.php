...
@foreach($testimonis as $testimoni)
    <div id="DeleteModal-{{ $testimoni->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
            <div class="flex flex-col items-center space-y-4">
                ...
                <!-- Detail testimoni (Konfirmasi Delete) -->
                <div class="bg-red-50 w-full border-2 border-red-500 rounded-lg p-4 text-sm text-gray-700">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2 flex items-center space-x-4">
                            ...
                            @endif
                            <div>
                                <p class="font-semibold text-gray-900 text-base">{{ $testimoni->anggota->nama }}</p>
                                <p class="text-gray-600 text-sm">
                                    {{ $testimoni->anggota->prodi }} • 
                                    Angkatan {{ $testimoni->anggota->angkatan }}<br>
                                    NIA: <span class="font-medium">{{ $testimoni->anggota->nia ?? '-' }}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <p class="font-semibold text-gray-900">Kesan:</p>
                ...
                <!-- Form Hapus -->
                <form id="deleteForm-{{ $testimoni->id }}" method="POST" action="{{ route('manage-testimoni.destroy', $testimoni->id ) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-3 mt-4">
                        <!-- Tombol Batal -->
                        <button type="button" onclick="closeDeleteModal('{{ $testimoni->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $testimoni->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
