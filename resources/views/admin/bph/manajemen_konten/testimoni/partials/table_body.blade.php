@forelse ($testimonis as $key => $testimoni)
<tr class="hover:bg-gray-50 text-left text-xs">
    ...
    <td class="px-6 py-4 break-words max-w-xs">
        {{ Str::limit($testimoni->kesan, 80) }}
    </td>

    <!-- Pesan -->
    <td class="px-6 py-4 break-words max-w-xs">
        {{ Str::limit($testimoni->pesan, 80) ?? '-' }}
    </td>
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center align-middle">
        <div class="flex justify-center items-center h-full space-x-2">
            <!-- Edit Button -->
            <button
                onclick="openUpdateModal('{{ $testimoni->id }}')"
            ...
            <!-- Delete Button -->
            <button
                onclick="openDeleteModal('{{ $testimoni->id }}')"
...
<tr class="hover:bg-gray-50">
    <td colspan="6" class="px-6 py-4 text-center text-gray-500 italic">
        <div class="flex flex-col items-center justify-center text-sm text-gray-500 space-y-1">
            @if ($testimonis->isEmpty() && !request()->filled('search') && !request()->filled('filter'))
                <!-- Icon Data Kosong -->
            ...
            @elseif ($testimonis->isEmpty() && request()->filled('search'))
                <!-- Icon Pencarian -->
            ...
            @elseif ($testimonis->isEmpty() && request()->filled('filter'))
                <!-- Icon Filter -->
