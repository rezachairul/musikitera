                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Update Lampiran (PDF/Word/Excel/PPT)
                            </label>
                            <input 
                                type="file" id="edit_file-{{ $kegiatan->id }}" name="lampiran" 
                            ...
                            <div id="filePreview-{{ $kegiatan->id }}" 
                                class="mt-2 border rounded-lg p-2 {{ $kegiatan->lampiran_path ? '' : 'hidden' }}">
                                
                                <p class="text-sm text-gray-600 mb-2 font-semibold">Preview:</p>

                                <!-- Preview Lama -->
                                <div id="oldPreviewContent-{{ $kegiatan->id }}"
                                    class="border rounded-lg p-3 bg-gray-50 {{ $kegiatan->lampiran_path ? '' : 'hidden' }}">
                                    @if($kegiatan->lampiran_path && pathinfo($kegiatan->lampiran_path, PATHINFO_EXTENSION) === 'pdf')
                                        <iframe src="{{ asset('storage/'.$kegiatan->lampiran_path) }}" 
                                                class="w-full h-96 border rounded-lg"></iframe>
                                    @elseif($kegiatan->lampiran_path)
                                        <p class="text-sm text-gray-700">
                                            {{ $manage_dokumen->original_filename ?? basename($kegiatan->lampiran_path) }}
                                        </p>
                                    @endif
                                </div>
                                
                                <!-- Preview Baru -->
                                <div id="previewContent-{{ $kegiatan->id }}" class="border rounded-lg p-3 bg-gray-50 hidden"></div>
                            </div>
                        </div>
