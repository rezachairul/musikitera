    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3 sm:p-6 m-3 sm:m-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-3 sm:gap-2">
            ...
            <div>
                ...
                <p class="text-gray-600 mt-1 text-sm">
                    Data {{ $title }} dikelola untuk menata halaman utama. <br>
                    Bagian ini menampilkan kutipan atau gambar utama sebagai identitas organisasi.
                </p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <!-- Export -->
                 <a href="{{ route('manage-hero.export', [
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="relative w-full sm:w-2/3">
                    ...
                    <input 
                        ...
                        data-target="ManageHeroTableBody"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-3 text-sm text-gray-700">
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                        <span>Total {{$title}}: <span class="font-semibold">{{ $totalHeroes }}</span></span>
                                    </div>
                                </td>
                            </tr>
                        </tfoot>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4 pt-4">
            ...
            <div class="text-sm text-gray-500 text-center sm:text-left">
                Menampilkan 
                <span class="font-medium">{{ $heroes->firstItem() }}</span> 
                sampai 
                <span class="font-medium">{{ $heroes->lastItem() }}</span> 
                dari 
                <span class="font-medium">{{ $heroes->total() }}</span> {{ $title }}
            </div>
            ...
            <div class="flex justify-center sm:justify-end">
                <nav class="inline-flex space-x-1 sm:space-x-2" aria-label="Pagination">
                    ...
                    {{-- Nomor Halaman --}}
                    @foreach ($heroes->getUrlRange(1, $heroes->lastPage()) as $page => $url)
                        @if ($page == $heroes->currentPage())
