...
@foreach($sejarahs as $sejarah)
    <div id="DeleteModal-{{ $sejarah->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
                ...
                <div class="bg-gray-50 w-full border-2 border-red-500 rounded-lg p-4 text-sm text-gray-700 space-y-3">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="w-20 h-20 rounded-lg overflow-hidden border">
                            @if ($sejarah->logo)
                                <img src="{{ asset('storage/' . $sejarah->logo) }}" 
                                    alt="Logo sejarah" 
                        ...
                        <div class="text-left">
                            <p><span class="font-semibold">Nama UKM:</span> {{ $sejarah->nama_ukm }} </p>
                            <p><span class="font-semibold">Tahun Awal:</span> {{ $sejarah->tahun_mulai }} </p>
                            <p><span class="font-semibold">Tahun Akhir:</span> {{ $sejarah->tahun_akhir ?? '(Masih Berlangsung)' }} </p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-4 text-justify">
                        <p><span class="font-semibold">Deskripsi:</span> {{ $sejarah->deskripsi }} </p>
                    </div>
                </div>

                <!-- Form Hapus -->
                <form id="deleteForm-{{ $sejarah->id }}" method="POST" action="{{ route('manage-sejarah.destroy', $sejarah->id ) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-3 mt-4">
                        <!-- Tombol Batal -->
                        <button type="button" onclick="closeDeleteModal('{{ $sejarah->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $sejarah->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
