<x-admin.bph.layouts>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3 sm:p-6 m-3 sm:m-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-3 sm:gap-2">
            ...
            <div>
                ...
                <p class="text-gray-600 mt-1 text-sm">
                    Data {{ $title }} digunakan untuk merekam perjalanan perubahan identitas dari masa ke masa. <br>
                </p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <!-- Export -->
                 <a href="{{ route('manage-sejarah.export', [
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="relative w-full sm:w-2/3">
                    ...
                    <input 
                        ...
                        data-url="{{ route('manage-sejarah.index') }}"
                        data-target="ManageSejarahTableBody"
            ...
            <div class="min-w-full inline-block align-middle">
                <div class="overflow-hidden border border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="ManageSejarahTableBody" class="bg-white divide-y divide-gray-200">
                            @include('admin.bph.tentang_ukmbsm.sejarah.partials.table_body')
                        </tbody>
                        <tfoot class="bg-gray-50">
                            <tr>
                                <td colspan="7" class="px-6 py-3 text-sm text-gray-700">
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                        <span>Total {{$title}}: <span class="font-semibold">{{ $totalsejarahs }}</span></span>
                                    </div>
                                </td>
                            </tr>
                        </tfoot>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4 pt-4">
            ...
            <div class="text-sm text-gray-500 text-center sm:text-left">
                Menampilkan 
                <span class="font-medium">{{ $sejarahs->firstItem() }}</span> 
                sampai 
                <span class="font-medium">{{ $sejarahs->lastItem() }}</span> 
                dari 
                <span class="font-medium">{{ $sejarahs->total() }}</span> {{ $title }}
            </div>
            ...
            <div class="flex justify-center sm:justify-end">
                <nav class="inline-flex space-x-1 sm:space-x-2" aria-label="Pagination">
                    ...
                    {{-- Nomor Halaman --}}
                    @foreach ($sejarahs->getUrlRange(1, $sejarahs->lastPage()) as $page => $url)
                        @if ($page == $sejarahs->currentPage())
                    ...
                    {{-- Tombol Selanjutnya --}}
                    @if ($sejarahs->hasMorePages())
                        <a href="{{ $sejarahs->nextPageUrl() }}" 
                        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                            Selanjutnya
                        </a>
                    @else
    ...
    <!-- Modals -->
    @include('admin.bph.tentang_ukmbsm.sejarah.create')
    @include('admin.bph.tentang_ukmbsm.sejarah.update')
    @include('admin.bph.tentang_ukmbsm.sejarah.delete')
