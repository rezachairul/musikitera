@forelse ($sejarahs as $key => $sejarah)
<tr class="hover:bg-gray-50 text-left text-xs">
    <!-- No -->
    <td class="px-6 py-4 whitespace-nowrap">
        {{ $sejarahs->firstItem() + $key }}
    </td>
    ...
    <!-- Deskripsi -->
    <td class="px-6 py-4 whitespace-nowrap">
        {{ Str::limit($sejarah->deskripsi, 50) }}
    </td>
    ...
    <!-- Tahun Berakhir -->
    <td class="px-6 py-4 whitespace-nowrap">
        {{ $sejarah->tahun_akhir ?? '(Masih Berlangsung)' }}
    </td>
    ...
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center align-middle">
        <div class="flex justify-center items-center h-full space-x-2">
            <!-- Edit Button -->
            <button
                onclick="openUpdateModal('{{ $sejarah->id }}')"
            ...
            <!-- Delete Button -->
            <button
                onclick="openDeleteModal('{{ $sejarah->id }}')"
...
<tr class="hover:bg-gray-50">
    <td colspan="7" class="px-6 py-4 text-center text-gray-500 italic">
        <div class="flex flex-col items-center justify-center text-sm text-gray-500 space-y-1">
            @if ($sejarahs->isEmpty() && !request()->filled('search') && !request()->filled('filter'))
                <!-- Icon Data Kosong -->
            ...
            @elseif ($sejarahs->isEmpty() && request()->filled('search'))
                <!-- Icon Pencarian -->
            ...
            @elseif ($sejarahs->isEmpty() && request()->filled('filter'))
                <!-- Icon Filter -->
